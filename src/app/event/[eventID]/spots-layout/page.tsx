...
export default async function SpotsLayoutPage({ params }: {
  params: { eventID: string }
}) {
    const { event, spots } = await getSpots(params.eventID)

    const rowsLetter = spots.map(spot => spot.name[0]);

    const uniqueRows = rowsLetter.filter(
      (row, index) => rowsLetter.indexOf(row) == index
    )
